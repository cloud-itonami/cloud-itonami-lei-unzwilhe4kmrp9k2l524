...
- `facts.edn` — GLEIF-registry facts (entity record, parent-reporting exceptions
  for both consolidation levels, direct-children list, ISINs, LEI issuer +
  accreditation, registration authority, ISO 20275 legal form), generated by
  `kbb --backend sci scripts/verify-facts.cljk --write`. Every value carries `:source/url` /
  `:source/http-status` / `:source/retrieved-at` provenance, and check mode
  (`kbb --backend sci scripts/verify-facts.cljk`) re-fetches every cited source and exits 1
  on drift.
